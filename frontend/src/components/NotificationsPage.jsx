  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400 dark:text-gray-300">Последние действия и предупреждения вашей учетной записи.</p>
      </motion.div>

      <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
        {loading ? (
          <div className="text-center text-gray-600 dark:text-gray-400 dark:text-gray-300">Загрузка уведомлений...</div>
        ) : error ? (
          <div className="text-center text-red-600 dark:text-red-400">{error}</div>
        ) : notifications.length === 0 ? (
          <div className="text-center py-16 text-gray-500 dark:text-gray-400 dark:text-gray-300 dark:text-gray-300">Уведомлений нет.</div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 dark:text-gray-300">{notification.message}</p>
                    </div>
                  </div>
                  <div className="text-right text-xs text-gray-500 dark:text-gray-400 dark:text-gray-300 dark:text-gray-300">
