    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-lg border border-gray-200 dark:border-gray-700">
          ...
          <p className="mt-3 text-gray-600 dark:text-gray-400">Только администратор может просматривать этот раздел.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">Список всех пользователей системы.</p>
      </motion.div>
                ...
                {users.map(user => (
                  <tr key={user.id} className="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/80">
                    ...
                    <td className="px-6 py-4 flex items-center gap-2">
                      ...
                      <span>{user.telegramChatId ? user.telegramChatId : 'Не подключен'}</span>
                    </td>
                  </tr>
                ))}
                {users.length === 0 && (
