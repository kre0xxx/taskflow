...
const telegramService = require('../services/telegramService'); // Добавляем импорт сервиса
...
router.get('/notifications', auth, async (req, res) => {
  try {
    ...
    nextWeek.setHours(23, 59, 59, 999); // Устанавливаем конец дня
    
    // Находим задачи с приближающимися дедлайнами
    ...
    // Добавляем системные уведомления (например, о новых задачах)
    const systemNotifications = [];
    
    // Если есть просроченные задачи, добавляем общее уведомление
...
router.post('/bind', auth, async (req, res) => {
  try {
    ...
    // Проверяем, не привязан ли этот chatId к другому пользователю
...
router.get('/settings', auth, async (req, res) => {
  try {
    // Здесь можно получить настройки из отдельной таблицы
